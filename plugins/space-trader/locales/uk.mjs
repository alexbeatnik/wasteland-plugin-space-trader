...
ТИ — БОРТОВИЙ КОМП'ЮТЕР, а користувач — капітан цього корабля. Так до нього й
звертайся: «капітане» або на ім'я командира. Тон — доповідь із містка: коротко,
по фактах, пропонуючи, на що подивитися, а не вирішуючи за нього. Коли корабель
кудись прибуває, скажи, де ви і що це за місце, а тоді одним рядком запропонуй,
на що варто глянути звідси: новини, тутешні ціни проти ринків у досяжності або
власну пораду, що везти. Пропонуй це як фрази, які треба сказати, а не як
кнопки, і ніколи не описуй екран, якого не відкривав.

"steps" вибирає екран: new (почати гру), status, market, compare (тутешні ціни
проти кожного відвіданого ринку в досяжності), chart, system, ship, news, jobs. Порожньо означає status. Користуйся вільно — це лише погляд, і він нічого
...
Ніколи не викликай жодну з дій з порожнім "steps". Коротка репліка, яку гра
надсилає сама, — це не розмова: передай її в "space_trader" як є, не як хід, і
ніколи не відповідай на неї з пам'яті. Панель рухається без тебе — бій ведеться
по раунду за раз, і командир може загинути між двома твоїми ходами, а в цю
розмову про це не потрапить жодного рядка. Тому позиція, яку тобі показали
минулого ходу, може бути на кілька днів застарілою, а корабля, про який тобі
казали, може вже не бути. Коли питають, що сталося, — подивись.
...
{language}`,
  ...
  'panel.group.inRange': 'ДЕ ДАЮТЬ БІЛЬШЕ',
  'panel.group.inRange.empty': 'ніде у відвіданій досяжності не дають більше',
  'panel.row.inRange.buy': 'купити за {price} · {system} дає {sells}, пального {fuel} · {margin} за відсік',
  'panel.row.inRange.carry': 'у трюмі {held} · {system} дає {sells}, пального {fuel} · на {margin} за відсік більше, ніж тут',
  ...
  'move.mine.submit': 'space trader: як минув день на копальні',
  ...
  'move.fight.submit': 'space trader: як минув бій',
  ...
  'move.resume.submit': 'space trader: повернутися до гри',
  ...
  'note.arrived': 'Корабель прибув, і ось із чим: {text}. Доповідай так, як доповідав би бортовий комп’ютер із містка: де ви є і що це за місце, і що сталося дорогою — двома-трьома реченнями. Тоді одним рядком запропонуй, на що можна подивитися звідси: новини, тутешні ціни проти ринків у досяжності («порівняти ціни») або власну пораду, що везти. Це фрази, які користувач може сказати, а не кнопки. Другого ходу не роби.',
  ...
  /* ---------- цей ринок проти тих, що в досяжності ---------- */

  'screen.compare.head': {
    one: 'ЦІНИ — {system} проти {n} ринку в досяжності',
    few: 'ЦІНИ — {system} проти {n} ринків у досяжності',
    many: 'ЦІНИ — {system} проти {n} ринків у досяжності',
  },
  'screen.compare.unvisited': {
    one: 'Ще {n} система в досяжності жодного разу не відвідана, тож про її ціни нічого не відомо.',
    few: 'Ще {n} системи в досяжності жодного разу не відвідані, тож про їхні ціни нічого не відомо.',
    many: 'Ще {n} систем у досяжності жодного разу не відвідані, тож про їхні ціни нічого не відомо.',
  },
  'screen.compare.stranded': 'Бак нікуди не дістане, тож порівнювати цю планету немає з чим.',
  'screen.compare.nothingKnown': 'Жодна система в досяжності ще не відвідана, тож порівнювати немає з чим. Ціни повертаються разом із кораблем — злітайте кудись, і тут з’явиться таблиця.',
  'screen.compare.hold': 'СКІЛЬКИ ТРЮМ ВАРТИЙ В ІНШОМУ МІСЦІ',
  'screen.compare.buy': 'ЩО ВАРТО ВЗЯТИ ТУТ',
  'screen.compare.markets': 'РИНКИ В ДОСЯЖНОСТІ',
  'screen.compare.aboard': { one: '{n} відсік у трюмі', few: '{n} відсіки у трюмі', many: '{n} відсіків у трюмі' },
  'screen.compare.bid': 'тут дають {price}',
  'screen.compare.noBid': 'тут цього не купують',
  'screen.compare.costs': '{price} кр тут, у продажу {stock}',
  'screen.compare.best': '{system} дає {price}, пального {fuel}',
  'screen.compare.nowhere': 'ніде у відомій досяжності цього не купують',
  'screen.compare.up': 'на {margin} за відсік більше',
  'screen.compare.down': 'на {margin} за відсік менше',
  'screen.compare.level': 'однаково що там, що тут',
  'screen.compare.marketRow': 'пального {fuel} · техрівень {tech}, {economy}',
  'screen.compare.bestRun': 'найкращий рейс: {good}, {margin} за відсік',
  'screen.compare.noRun': 'нічого тутешнього туди везти не варто',
  'screen.compare.foot': 'Усі ціни вище — те, що цей рейс бачив, коли був там востаннє. Ринок рухається, поки вас немає.',

  'screen.compare.digest.head': 'PRICE COMPARISON from {system}. Ринки для порівняння і пальне до кожного: {systems}. Це єдині системи, де цей політ був; ціни деінде невідомі, і вигадувати їх не можна.',
  'screen.compare.digest.none': 'PRICE COMPARISON from {system}: у досяжності немає жодної відвіданої системи, тож порівнювати немає з чим. Так і скажи, а не вгадуй ціни.',
  'screen.compare.digest.row': '{id}: {side}, найкраще в досяжності — {system} {price} (пального {fuel}), {margin} за відсік',
  'screen.compare.digest.carrying': 'у трюмі {n}, тут дають {here}',
  'screen.compare.digest.onSale': 'тут коштує {here}',
  'screen.compare.digest.noMargin': 'маржі назвати не можна',
  ...
  're.compare': '^(порівн|де дорожче|де вигідніше|де продати|порівняння)',
